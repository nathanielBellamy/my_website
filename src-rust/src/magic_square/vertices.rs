...
const ORIGIN: Vertex = [0.0, 0.0, 0.0];
...
impl Vertices {
    ...
    pub fn hexagon(buffer: &[f32; 2], radius: f32) -> Vertices {
        let mut vertices = Vertices::new();

        let center_x = buffer[0];
        let center_y = buffer[1];
        let center: Vertex = [buffer[0], buffer[1], 0.0];
        
        vertices.set_next(center);
        vertices.set_next([center_x - 0.05, center_y, 0.0]);

        vertices.set_next([center_x - 0.05, center_y, 0.0]);
        vertices.set_next([center_x - 0.05, center_y + 0.05, 0.0]);

        vertices.set_next([center_x - 0.05, center_y + 0.05, 0.0]);
        vertices.set_next([center_x, center_y + 0.05, 0.0]);
