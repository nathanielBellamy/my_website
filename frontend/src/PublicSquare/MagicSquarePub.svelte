<script lang="ts" type="module">
  ...
  import { intoLfoDestination } from '../MagicSquare/ControlModules/LfoDestination'
  import type { LfoDestination } from '../MagicSquare/ControlModules/LfoDestination'
  import { intoLfoShape } from '../MagicSquare/ControlModules/LfoShape'
  import type { LfoShape } from '../MagicSquare/ControlModules/LfoShape'
  import type { MouseTracking } from '../MagicSquare/ControlModules/MouseTracking'
  import MouseTrackingContainer from '../MagicSquare/ControlModules/MouseTracking.svelte'
  import Rotation from  '../MagicSquare/ControlModules/Rotation.svelte'
  import type { StorageSettings } from '../MagicSquare/StorageSettings'
  import Translation from '../MagicSquare/ControlModules/Translation.svelte'
  import { WasmInputId } from '../MagicSquare/WasmInputId'
  import { intoTransformOrder, TransformOrder } from '../MagicSquare/ControlModules/TransformOrder'
  ...
  function setInitialDrawPatternVars(settings: any) {
    drawPatternType = intoDrawPatternType(settings.draw_pattern_type)
    drawPatternCount = settings.draw_pattern_count
    drawPatternOffset = settings.draw_pattern_offset
    drawPatternSpeed = settings.draw_pattern_speed
  }
  ...
  function setInitialColorVars(settings: any) {
    colors = settings.colors.map((x: number[]) => convertRgba(x, 'up'))
    colorDirection = intoColorDirection(settings.color_direction)
    colorSpeed = settings.color_speed
  }
  ...
  function setInitialGeometryVars(settings: any) {
    radiusBase = round2(settings.radius_base)
    radiusStep = round2(settings.radius_step)
    shapes = settings.shapes.map((x:string) => intoShape(x))
    transformOrder = intoTransformOrder(settings.transform_order)
  }
  ...
  function setInitialLfoVars(settings: any) {
    lfo1Active = settings.lfo_1_active
    lfo1Amp = round2(settings.lfo_1_amp)
    lfo1Dest = intoLfoDestination(settings.lfo_1_dest)
    lfo1Freq = round2(settings.lfo_1_freq)
    lfo1Phase = round2(settings.lfo_1_phase)
    lfo1Shape = intoLfoShape(settings.lfo_1_shape)

    lfo2Active = settings.lfo_2_active
    lfo2Amp = round2(settings.lfo_2_amp)
    lfo2Dest = intoLfoDestination(settings.lfo_2_dest)
    lfo2Freq = round2(settings.lfo_2_freq)
    lfo2Phase = round2(settings.lfo_2_phase)
    lfo2Shape = intoLfoShape(settings.lfo_2_shape)

    lfo3Active = settings.lfo_3_active
    lfo3Amp = round2(settings.lfo_3_amp)
    lfo3Dest = intoLfoDestination(settings.lfo_3_dest)
    lfo3Freq = round2(settings.lfo_3_freq)
    lfo3Phase = round2(settings.lfo_3_phase)
    lfo3Shape = intoLfoShape(settings.lfo_3_shape)

    lfo4Active = settings.lfo_4_active
    lfo4Amp = round2(settings.lfo_4_amp)
    lfo4Dest = intoLfoDestination(settings.lfo_4_dest)
    lfo4Freq = round2(settings.lfo_4_freq)
    lfo4Phase = round2(settings.lfo_4_phase)
    lfo4Shape = intoLfoShape(settings.lfo_4_shape)
  }
  ...
  function setInitialTranslationVars(settings: any) {
    translationXBase = round2(settings.translation_x_base)
    translationXSpread = round2(settings.translation_x_spread)
    translationYBase = round2(settings.translation_y_base)
    translationYSpread = round2(settings.translation_y_spread)
    // translationZ = settings.translation_z
  }

  let mouseTracking: MouseTracking

  function setInitialMouseTracking(settings: any) {
    mouseTracking = settings.mouse_tracking
  }
  ...
  function setInitialRotationVars(settings: any) {
    pitchBase = round2(settings.y_rot_base)
    pitchSpread = round2(settings.y_rot_spread)
    pitchX = round2(settings.x_axis_y_rot_coeff)
    pitchY = round2(settings.y_axis_y_rot_coeff)
    rollBase = round2(settings.x_rot_base)
    rollSpread = round2(settings.x_rot_spread)
    rollX = round2(settings.x_axis_x_rot_coeff)
    rollY = round2(settings.y_axis_x_rot_coeff)
    yawBase = round2(settings.z_rot_base)
    yawSpread = round2(settings.z_rot_spread)
    yawX = round2(settings.x_axis_z_rot_coeff)
  ...
  async function run() {
    if (!hasBeenDestroyed) { 
      ...
      settings = await PubSq.run(
        `${import.meta.env.VITE_BASE_URL}/old-site/`,
