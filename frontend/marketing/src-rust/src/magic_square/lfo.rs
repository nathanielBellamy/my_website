        ...
        if self.active {
            match self.dest {
                LfoDestination::PitchBase => {
                    settings.y_rot_base = settings.y_rot_base + self.eval(t) * 3.14
                }
                LfoDestination::PitchSpread => {
                    settings.y_rot_spread = settings.y_rot_spread + self.eval(t) * 0.3
                }
                LfoDestination::PitchX => {
                    settings.x_axis_y_rot_coeff = settings.x_axis_y_rot_coeff + self.eval(t)
                }
                LfoDestination::PitchY => {
                    settings.y_axis_y_rot_coeff = settings.y_axis_y_rot_coeff + self.eval(t)
                }
                LfoDestination::RollBase => {
                    settings.x_rot_base = settings.x_rot_base + self.eval(t) * 3.14
                }
                LfoDestination::RollSpread => {
                    settings.x_rot_spread = settings.x_rot_spread + self.eval(t) * 0.3
                }
                LfoDestination::RollX => {
                    settings.x_axis_x_rot_coeff = settings.x_axis_x_rot_coeff + self.eval(t)
                }
                LfoDestination::RollY => {
                    settings.y_axis_x_rot_coeff = settings.y_axis_x_rot_coeff + self.eval(t)
                }
                LfoDestination::YawBase => {
                    settings.z_rot_base = settings.z_rot_base + self.eval(t) * 3.14
                }
                LfoDestination::YawSpread => {
                    settings.z_rot_spread = settings.z_rot_spread + self.eval(t) * 0.3
                }
                LfoDestination::YawX => {
                    settings.x_axis_z_rot_coeff = settings.x_axis_z_rot_coeff + self.eval(t)
                }
                LfoDestination::YawY => {
                    settings.y_axis_z_rot_coeff = settings.y_axis_z_rot_coeff + self.eval(t)
                }
                LfoDestination::RadiusBase => {
                    settings.radius_base = settings.radius_base + self.eval(t)
                }
                LfoDestination::RadiusStep => {
                    settings.radius_step = settings.radius_step + self.eval(t)
                }
                LfoDestination::TranslationXBase => {
                    settings.translation_x_base = settings.translation_x_base + self.eval(t);
                }
                LfoDestination::TranslationXSpread => {
                    settings.translation_x_spread = settings.translation_x_spread + self.eval(t);
                }
                LfoDestination::TranslationYBase => {
                    settings.translation_y_base = settings.translation_y_base + self.eval(t);
                }
